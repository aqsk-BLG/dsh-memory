...
import { collectSemanticRankingText } from './ranking-output.ts'
import { parseExactEvidenceRanking, rankTournament } from './ranking-policy.ts'
import {
...
async function rankBatch(
  ...
  const text = collectSemanticRankingText(assembler.blocks())
...
async function rankAll(
  ...
  return rankTournament(candidates, config.semanticBatchSize, limit, (batch, batchLimit) =>
    rankBatch(
      llm,
      target,
      query,

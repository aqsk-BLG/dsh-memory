...
function apply$4(ctx, config) {
	...
	if (config.seedMissingFiles) {
		...
		for (const path of before) ctx.logger.info(`dsh-memory: seeded a starter template at ${JSON.stringify(path)} (edit freely; it is never overwritten)`);
	}
	ctx.effect(() => ctx.root.on("agent/created", ({ agent }) => {
...
const MIN_HOST_VERSION = "0.1.0-rc.7";
/**
...
function readPackageVersion(packageName) {
	try {
		let dir = dirname(createRequire(import.meta.url).resolve(packageName));
		for (let depth = 0; depth < 10; depth++) {
			const candidate = join(dir, "package.json");
			if (existsSync(candidate)) {
				const manifest = JSON.parse(readFileSync(candidate, "utf8"));
				if (manifest.name === packageName && typeof manifest.version === "string" && manifest.version.length > 0) return manifest.version;
			}
...
function parseLooseVersion(raw) {
...
*/
function assessHostVersion(info) {
...
function describe(info) {
	if (info === void 0) return "no DeepSeek Harness version could be detected";
	const origin = info.source === "env" ? "DSH_VERSION environment variable" : `package manifest of ${info.packageName ?? "an installed @deepseek-ai package"}`;
...
*/
function unsupportedHostMessage(pluginVersion) {
...
function readOwnVersion() {
	try {
		...
		for (let depth = 0; depth < 10; depth++) {
			...
			if (existsSync(candidate)) {
				const manifest = JSON.parse(readFileSync(candidate, "utf8"));
				if ((manifest.name === "dsh-file-memory" || manifest.name === "dsh-memory") && typeof manifest.version === "string") return manifest.version;
			}
			const parent = dirname(dir);
			if (parent === dir) break;
			dir = parent;
...
*/
function enforceHostVersionGate(logger, mode, pluginVersion = readOwnVersion()) {
	if (mode === "off") return;
	...
	if (assessment.reason === "below-min") {
		const message = `${unsupportedHostMessage(pluginVersion)} (${describe(info)}). Upgrade DeepSeek Harness, pin dsh-memory to v1.1.x, or set versionGate to warn/off if you maintain a compatible fork.`;
		if (mode === "error") throw new Error(message);
		logger.warn(`dsh-memory: ${message}`);
		return;
	}
	logger.warn(`dsh-memory: ${describe(info)}; the host version gate cannot verify compatibility. Continuing because nothing is provably missing (${assessment.reason}).`);
}

//#endregion
//#region src/panel-files.ts
/**
...
function looksLikeDshSourceRoot(dir) {
	return existsSync(join(dir, "AGENTS.md")) && existsSync(join(dir, "packages")) && existsSync(join(dir, ".git"));
}
...
function managedState(id, content) {
	const spec = FILES.find((file) => file.id === id);
	if (spec === void 0 || !spec.managed) return "none";
	return inspectManagedRegion(content).valid ? "valid" : "malformed";
}
function toInfo(id, name$6, path, content) {
	if (content === void 0) return {
		...
		bytes: 0,
		chars: 0,
		managed: id === "user" || id === "memory" ? "valid" : "none"
	};
	return {
		id,
		name: name$6,
...
function memoryPanelFilePath(home, id) {
	const spec = FILES.find((file) => file.id === id);
	if (spec === void 0) throw new Error(`unknown memory panel file: ${id}`);
	if (spec.root === "source") {
		const source = resolveDshSourceRoot();
		if (source !== void 0) return join(source, spec.name);
	}
	return join(home, spec.name);
}
/** List catalog files without reading huge bodies twice. */
async function listMemoryPanelFiles(home) {
	const rows = [];
	for (const spec of FILES) {
		const path = memoryPanelFilePath(home, spec.id);
		if (!existsSync(path)) {
			rows.push(toInfo(spec.id, spec.name, path, void 0));
			continue;
		}
		try {
			statSync(path);
...
async function readMemoryPanelFile(home, id) {
	const spec = FILES.find((file) => file.id === id);
	if (spec === void 0) throw new Error(`unknown memory panel file: ${id}`);
...
*/
async function writeMemoryPanelFile(home, id, content, expectedHash) {
	const spec = FILES.find((file) => file.id === id);
	...
	const path = memoryPanelFilePath(home, id);
	if (spec.managed && !inspectManagedRegion(content).valid) return {
		ok: false,
	...
	try {
		return await withFileLock(path, async () => {
			...
			if (hashContent(currentText) !== expectedHash) return {
				ok: false,
				reason: "conflict",
			...
			return {
				ok: true,
				file: await readMemoryPanelFile(home, id)
			};
		});
	} catch (error) {
		return {
...
*/
function newestConsolidation(home) {
	const dir = join(home, "memory", "consolidation");
	if (!existsSync(dir)) return void 0;
	let best;
	for (const name$6 of readdirSync(dir)) {
		if (!name$6.endsWith(".json")) continue;
		const path = join(dir, name$6);
		try {
			const st = statSync(path);
			if (best === void 0 || st.mtimeMs > best.mtime) best = {
				path,
				mtime: st.mtimeMs,
				sessionId: name$6.slice(0, -5)
			};
		} catch {}
	}
	if (best === void 0) return void 0;
	try {
		const raw = JSON.parse(readFileSync(best.path, "utf8"));
		return {
...
async function readMemoryPanelStatus(home) {
	...
	return {
		...
		...newestConsolidation(home) === void 0 ? {} : { lastConsolidation: newestConsolidation(home) }
	};
}

//#endregion
//#region src/panel-http.ts
/** Prefix owned by this plugin. */
const MEMORY_PANEL_API_PREFIX = "/api/plugins/dsh-file-memory";
...
function fileIdFrom(pathname) {
	const prefix = `${MEMORY_PANEL_API_PREFIX}/files/`;
	if (!pathname.startsWith(prefix)) return void 0;
	const id = pathname.slice(prefix.length);
	return FILE_IDS.has(id) ? id : void 0;
}
/** Handle one panel request. Unknown paths 404 so the SPA fallback stays unused. */
async function handleMemoryPanelRequest(home, req, res) {
	const path = new URL(req.url ?? "/", "http://127.0.0.1").pathname.replace(/\/$/u, "") || "/";
	...
	if (id !== void 0 && method === "PUT") {
		...
		if (typeof payload.content !== "string" || typeof payload.expectedHash !== "string") {
			json(res, 400, {
				ok: false,
				reason: "io",
				message: "content and expectedHash are required"
			});
			return;
		}
...
function registerMemoryPanelHttp(ctx, home) {
	ctx.inject(["webServer"], (sctx) => {
		const dispose = sctx.webServer.register({
			kind: "prefix",
			path: MEMORY_PANEL_API_PREFIX,
			handler: (req, res) => {
				handleMemoryPanelRequest(home, req, res).catch((error) => {
...
const MEMORY_SETTINGS_NAMESPACE = settingsNamespace("memory");
/** Resolve the same home the rest of the facade uses. */
function resolveMemoryPanelHome(dshHome) {
...
*/
function applyMemoryPanel(ctx, schema, config) {

...
function renderMemoryScope(ctx, sessionId, options) {
	...
	const projectMemory = readBounded(target.curatedMemoryFile, options.projectMemoryBudgetChars);
...
const SUCCESSFUL_WATERMARK_STATUSES = new Set([
	"applied",
	"noop",
	"proposed",
	"partial"
...
function appendDaily(content, section) {
	if (section.length === 0) return content;
	return `${content.length === 0 ? "" : `${content.replace(/\s+$/u, "")}\n\n`}${section}`;
}
async function commitFile(ctx, agent, snapshot, file, candidates, config, throughSeq, now, signal) {
	...
	try {
		...
		const next = file.target === "daily" ? appendDaily(file.content, dailySection) : rewriteManagedRegion(file.content, entries);
...
function lastWatermark(events) {
	const event = events.findLast((candidate) => candidate.type === "memory/consolidation-result" && SUCCESSFUL_WATERMARK_STATUSES.has(candidate.data.status));
...
async function consolidate(ctx, agent, config, home, lifetime, signal) {
	...
	try {
		...
		signal.throwIfAborted();
		const outcomes = await Promise.all(snapshot.files.map((file) => commitFile(ctx, agent, snapshot, file, candidates, config, throughSeq, now, signal)));
...
function parseRanking(text, candidates, limit) {
	...
	if (candidates.length > 0 && ranked.length === 0) throw new Error("semantic ranking returned no valid candidate hits");
	return ranked;
}
...
async function rankAll(llm, target, query, candidates, limit, config, agent, signal) {
	if (candidates.length <= config.semanticBatchSize) return rankBatch(llm, target, query, candidates, limit, config.semanticMaxTokens, agent, signal);
	const survivors = [];
	for (let start = 0; start < candidates.length; start += config.semanticBatchSize) {
		...
		const ranked = await rankBatch(llm, target, query, batch, Math.min(limit, batch.length), config.semanticMaxTokens, agent, signal);
...
overwrite a file changed after its snapshot. \`IDENTITY.md\` and \`SOUL.md\` are never targets.

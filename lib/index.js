...
function parseConsolidationOutput(text, config) {
	...
	const keys = Object.keys(record$1).sort();
	...
	return {
		user: normalizeEntries(record$1.user, "user", config.userBudgetChars),
		global: normalizeEntries(record$1.global, "global", config.globalBudgetChars),
		project: normalizeEntries(record$1.project, "project", config.projectBudgetChars),
		daily: normalizeEntries(record$1.daily, "daily", config.dailyBudgetChars)
	};
}
...
async function commitFile(ctx, agent, snapshot, file, candidates, config, throughSeq, now, explicitForget, signal) {
	...
	try {
		...
		const plan = file.target === "daily" ? void 0 : planManagedRewrite(file.managedEntries, entries, config.maxDeletionRatio, explicitForget);
		let guardedDeletionReason;
		if (plan !== void 0) {
			diff = {
			...
			if (plan.blocked) {
				...
				if (config.mode === "proposal" || plan.diff.added.length === 0) return outcome("proposed", guardedDeletionReason);
				const budget = file.target === "user" ? config.userBudgetChars : file.target === "global" ? config.globalBudgetChars : config.projectBudgetChars;
				if (managedRegionCodePoints(plan.entries) > budget) return outcome("proposed", `${guardedDeletionReason}; ${plan.diff.added.length} safe additions were not written because retaining guarded entries would exceed the ${budget}-character budget`);
		...
		if (next === file.content) return guardedDeletionReason === void 0 ? outcome("noop") : outcome("proposed", guardedDeletionReason);
		...
		return await withFileLock(file.path, async () => {
			...
			return guardedDeletionReason === void 0 ? outcome("applied") : outcome("proposed", `${guardedDeletionReason}; applied ${plan?.diff.added.length ?? 0} safe additions while retaining guarded entries`);
		});
	} catch (error) {
	...
	try {
		...
		if (malformed.length > 0) {
			...
			agent.session.append("memory/consolidation-result", {
				...
				outcomes: malformed,
				retry: retryMetadata(events, currentSnapshot, throughSeq, "malformed-managed-region", "file-change", config, now),
		...
		agent.session.append("memory/consolidation-request", {
			...
			files: currentSnapshot.files.map(({ target, path, contentHash: contentHash$1, existed, managedRegionValid }) => ({
				target,
				path,
		...
		const outcomes = await Promise.all(currentSnapshot.files.map((file) => commitFile(ctx, agent, currentSnapshot, file, candidates, config, throughSeq, now, explicitForget, signal)));
		...
		const retry = retryable.length === 0 ? void 0 : retryMetadata(events, currentSnapshot, throughSeq, JSON.stringify(retryable.map(({ target, status: targetStatus, error }) => ({
			target,
			status: targetStatus,
			error
